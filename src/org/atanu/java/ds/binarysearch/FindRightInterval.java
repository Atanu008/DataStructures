...
//LeetCode 436
// Video : https://www.youtube.com/watch?v=FfLKadZK0oY
public class FindRightInterval {

    //Brute Force
    //O(N2)

    // Doubt : [[1,1],[3,4]] : Output [0,-1] , it should be [1 , -1] I guess . i and j index should not be same I guess
    ...
    // In our case, we store the data such that the start point of an interval acts as the Key
    // Key and the index corresponding to the interval acts as the value, since we are concerned with data sorted based on the start points,
    // as discussed in previous approaches. Every element of the intervals
    // intervals array is stored in the TreeMap
    // Now, we choose each element of the intervals
    // intervals array and make use of a function TreeMap.ceilingEntry(end_point)
    // to obtain the element in the TreeMap with itsKey
    // Key just larger than the end_point of the currently chosen interval.
    // The function ceilingEntry(Key) returns the element just with its Key
    ...
    public int[] findRightIntervalV2(int[][] intervals) {
        ...
        for(int i = 0; i < intervals.length; i++){
            Map.Entry<Integer, Integer> ceilingEntry = starts.ceilingEntry(intervals[i][1]);
